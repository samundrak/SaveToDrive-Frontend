...
class RemoteUploadForm extends React.Component {
  ...
  getServices() {
    return (
      ...
      >
        {this.props.connections.filter((c) => c.status).map((connection) => (
          <Option key={connection._id} title={connection.profile.email || connection.profile.name}>
            ...
            {connection.profile.name} &lt;{startCase(connection.service_type)}&gt;
          </Option>
        ))}
      </Select>
    );
  ...
  render() {
    ...
    return (
      <Row>
        <Form>
          <Form.Item>
            <Urls urls={this.props.upload.urls} handleChange={this.props.handleUrlChanges} handleRemove={this.props.handleUrlRemove} />
            <Button type="primary" onClick={this.props.handleAddUrlField}>
              <Icon type="upload" />Add Url
            </Button>
            {this.getServices()}
          </Form.Item>
          ...
          <Form.Item>
            <div style={{ width: '100%' }}>
              <Row>
                ...
                <Col span={12}>
                  <Checkbox defaultChecked={upload.isEmail} checked={upload.isFilename} onClick={this.props.handleChange('isFilename')}>
                    Change File Name
                  </Checkbox>
